...
extern const struct i2c_dt_spec dev_i2c_air;
...
BME68X_INTF_RET_TYPE bme68x_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    return i2c_burst_read_dt(&dev_i2c_air, reg_addr, reg_data, len);
}
...
BME68X_INTF_RET_TYPE bme68x_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    return i2c_burst_write_dt(&dev_i2c_air, reg_addr, reg_data, len);
}
...
void bme68x_check_rslt(const char api_name[], int8_t rslt)
{
    switch (rslt)
    {
        ...
        case BME68X_E_NULL_PTR:
            printk("API name [%s]  Error [%d] : Null pointer\r\n", api_name, rslt);
            break;
        case BME68X_E_COM_FAIL:
            printk("API name [%s]  Error [%d] : Communication failure\r\n", api_name, rslt);
            break;
        case BME68X_E_INVALID_LENGTH:
            printk("API name [%s]  Error [%d] : Incorrect length parameter\r\n", api_name, rslt);
            break;
        case BME68X_E_DEV_NOT_FOUND:
            printk("API name [%s]  Error [%d] : Device not found\r\n", api_name, rslt);
            break;
        case BME68X_E_SELF_TEST:
            printk("API name [%s]  Error [%d] : Self test error\r\n", api_name, rslt);
            break;
        case BME68X_W_NO_NEW_DATA:
            printk("API name [%s]  Warning [%d] : No new data found\r\n", api_name, rslt);
            break;
        default:
            printk("API name [%s]  Error [%d] : Unknown error code\r\n", api_name, rslt);
            break;
    }
}
